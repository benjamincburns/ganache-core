...
function ForkedBlockchain(options) {
  ...
  // forest of address-based tries, one per account
  // when changes happen to an account, they are pushed into one of the tries stored in this cache
...
ForkedBlockchain.prototype.createVM = function(options) {
  var self = this;
  var vm = BlockchainDouble.prototype.createVM.call(self, options);

  // Unfortunately forking requires a bit of monkey patching, but it gets the job done.
  vm.stateManager._lookupStorageTrie = self.lookupStorageTrie.bind(self);
  vm.stateManager.cache._lookupAccount = self.getAccount.bind(self);
  vm.stateManager.getContractCode = self.getCode.bind(self);
  vm.stateManager.putContractCode = self.putCode.bind(self);
