...
BlockchainDouble.prototype.createVM = function(options) {
  var self = this;
  var vm = options.vm || new VM({
    state: options.stateTrie || self.stateTrie,
    blockchain: {
...
BlockchainDouble.prototype.putAccount = function(account, address, callback) {
  var self = this;
  var vm = self.createVM(self.options);
...
BlockchainDouble.prototype.processBlock = function(block, commit, callback) {
  ...
  self._processBlock(block, vm, commit, callback);
}

BlockchainDouble.prototype._processBlock = function(block, vm, commit, callback) {
  var self = this;
...
BlockchainDouble.prototype.processTransactionTrace = function(hash, params, callback) {
  ...
  this.getTransactionReceipt(target_hash, function(err, receipt) {
    ...
    self.getBlock(targetBlock.header.parentHash, function(err, parent) {
      ...
      var vmOptions = clone(self.options);
      vmOptions.stateTrie = self.stateTrie.copy();
      vmOptions.stateTrie.root = parent.header.stateRoot;

      vm = self.createVM(vmOptions);
      ...
      self.createBlock(parent, function(err, block) {
        ...
        // fake out all the rest of the block metadata
        block.header = clone(targetBlock.header, false);

        // #2 - Process the block without committing the data.
        self._processBlock(block, vm, false, function(err, transactions, results) {
...
BlockchainDouble.prototype.getAccount = function(address, number, callback) {
  ...
  this.getBlock(number, function(err, block) {
    ...
    var trie = self.createStateTrie(self.stateTrie.db, block.header.stateRoot);
...
BlockchainDouble.prototype.getStorage = function(address, position, number, callback) {
  ...
  this.getBlock(number, function(err, block) {
    ...
    trie.get(utils.toBuffer(address), function(err, data) {
      ...
      trie = self.createStateTrie(self.stateTrie.db, account.stateRoot);
